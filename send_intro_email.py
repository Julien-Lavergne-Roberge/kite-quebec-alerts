...
SUBJECT = "🪁 Nouveau service météo kite (c'est moi, ton fils préféré)"

BODY = f"""Salut!

T'es l'heureux bénéficiaire d'un service de notification météo-kitesurf
entièrement gratuit, personnalisé, et d'une générosité sans borne. 🪁

Le concept: quand les astres s'alignent -  météo, vent dans la bonne
direction, température clémente, pas de pluie forte, congé en vue demain - un courriel part
...
Quelques précisions honnêtes:
  ...
  • Non, il n'y a pas de bouton « désabonnement » officiel — mais envoie-moi
    juste un texto si tu es tanné et je l'arrêterai
  • Oui, j'ai codé ça avec l'aide de Claude (l'IA d'Anthropic). On aura bientot plus de job.
  • Le dashboard est consultable en tout temps:
    {DASHBOARD_URL}

Ce courriel-ci n'est qu'un test d'introduction. Les vrais courriels viendront
SEULEMENT quand les conditions sont vraiment excellentes le lendemain — pas tous
